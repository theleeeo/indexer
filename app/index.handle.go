...
func (a *App) handleRebuild(ctx context.Context, p RebuildPayload) error {
	logger := slog.With(slog.String("jobType", "rebuild"), slog.String("type", p.ResourceType), slog.String("id", p.ResourceID))

	doc, err := a.builder.Build(ctx, p.ResourceType, p.ResourceID)
	if err != nil {
	...
	if err := a.es.Upsert(ctx, indexName, p.ResourceID, doc); err != nil {
...
func (a *App) handleDelete(ctx context.Context, p RebuildPayload) error {
	logger := slog.With(slog.String("jobType", "delete"), slog.String("type", p.ResourceType), slog.String("id", p.ResourceID))

	indexName := p.ResourceType + "_search"
	if err := a.es.Delete(ctx, indexName, p.ResourceID); err != nil {
	...
	if err := a.st.RemoveResource(ctx, model.Resource{Type: p.ResourceType, Id: p.ResourceID}); err != nil {
