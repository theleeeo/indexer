...
func (t *TestSuite) Test_Resource_CRUD_OneIndex() {
	...
	t.app.SetBuilder(projection.NewBuilder(t.fakeProvider, DefaultResourceConfig, t.st))
	...
	t.Run("create resources", func() {
		err := t.app.RegisterChange(t.T().Context(), source.Notification{
			ResourceType: "a",
			ResourceID:   "1",
...
func (t *TestSuite) Test_RelatedRelations_FullGraph() {
	...
	t.app.SetBuilder(projection.NewBuilder(t.fakeProvider, RelatedResourceConfig, t.st))
	...
	for _, n := range []source.Notification{
		...
		err := t.app.RegisterChange(t.T().Context(), n)
		t.Require().NoError(err)
	}
	t.worker.Drain(t.T().Context())
